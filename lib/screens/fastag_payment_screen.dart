...
class FastagPaymentScreen extends StatefulWidget {
  final Map<String, dynamic> billData;
  final Map<String, dynamic> fastagData;
  final bool isB2B; // ✅ Ab ye REQUIRED hai, galti ki koi gunjaish nahi

  const FastagPaymentScreen({
    super.key,
    required this.billData,
    required this.fastagData,
    required this.isB2B, // 🚀 B2B privacy ke liye
  });

  @override
  State<FastagPaymentScreen> createState() => _FastagPaymentScreenState();
}

class _FastagPaymentScreenState extends State<FastagPaymentScreen> {
  bool _isPaying = false;
  final double _walletBalance = 3500.00;

  double get _rechargeAmount {
    final raw = widget.billData['rechargeAmount'].toString().replaceAll(',', '');
    return double.tryParse(raw) ?? 0.0;
  }

  bool get _hasSufficientBalance => _walletBalance >= _rechargeAmount;

  // ✅ JADOO: Dynamic Theme based on User Role (B2B = Green, B2C = Blue)
  List<Color> get _themeGradient {
    return widget.isB2B
        ? [const Color(0xFF00695C), const Color(0xFF009688)] // Paysaral Forest Green
        : [const Color(0xFF1E3C72), const Color(0xFF2A5298)]; // Premium Blue for B2C
  }
  ...
  // ✅ JADOO: Indian Currency Comma Formatter (E.g., 3500.00 -> 3,500.00)
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Recharge Successful!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: Colors.black87)),
              const SizedBox(height: 6),
              Text(
                '₹${widget.billData['rechargeAmount']} added to ${widget.billData['vehicleNo']}',
              ...
              Container(
                ...
                child: Column(
                  children: [
                    _receiptRow('Vehicle No.', widget.billData['vehicleNo']),
                    _receiptRow('Fastag Bank', widget.fastagData['name']),
                    _receiptRow('Recharge Amount', '₹${widget.billData['rechargeAmount']}'),
                    _receiptRow('Date & Time', '27 Mar 2026, 10:15 AM'),
                    _receiptRow('Txn ID', 'FSTG${DateTime.now().millisecondsSinceEpoch.toString().substring(5)}'),

                    // ✅ 100% STRICT B2B PRIVACY LOGIC:
                    // Agar user B2B (Retailer) NAHI hai, tabhi cashback dikhega.
                    // Retailer ke liye ye line gayab ho jayegi!
                    if (!widget.isB2B)
                      _receiptRow('Cashback Earned', '+₹10.00', valueColor: Colors.orange.shade700),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Navigator.of(context)..pop()..pop()..pop(); // Go back to Home/Operator screen
                  },
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            // ✅ JADOO: Scroll hata kar sirf Expanded aur Spacer lagaya hai, ab screen 100% lock rahegi!
            Expanded(
              child: Padding(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Container(
                            width: 52, height: 52,
                            decoration: BoxDecoration(color: (widget.fastagData['color'] as Color).withOpacity(0.12), borderRadius: BorderRadius.circular(14)),
                            child: Icon(widget.fastagData['icon'] as IconData, color: widget.fastagData['color'] as Color, size: 26),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(widget.fastagData['name'], style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.black87)),
                                const SizedBox(height: 3),
                                Text(widget.fastagData['fullName'], style: TextStyle(fontSize: 12, color: Colors.grey.shade500), maxLines: 1, overflow: TextOverflow.ellipsis),
                              ],
                            ),
                          ),
                        ],
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                Text('Recharge Details', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: _primaryThemeColor)),
                              ],
                            ),
                          ),
                          Padding(
                            ...
                            child: Column(
                              children: [
                                _summaryRow('Owner Name', widget.billData['customerName']),
                                _summaryRow('Vehicle No.', widget.billData['vehicleNo']),
                                _summaryRow('Current Balance', '₹${widget.billData['currentBalance']}'),
                                ...
                                Row(
                                  ...
                                  children: [
                                    const Text('Recharge Amount', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.black87)),
                                    Text('₹${widget.billData['rechargeAmount']}', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: _primaryThemeColor)),
                                  ],
                                ),
                              ],
                            ),
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          // ✅ JADOO: Wallet balance me Comma aa gaya!
                          Text('₹${_formatCurrency(_walletBalance)}', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: _hasSufficientBalance ? Colors.green.shade600 : Colors.red.shade400)),
                        ],
                      ),
                    ),

                    // ✅ Spacer lagane se Pay button aur Logo hamesha screen ke bottom me lock rahenge
                    const Spacer(),
                          ...
                          child: Container(
                            ...
                            child: _isPaying
                              ...
                              children: [
                                ...
                                const SizedBox(width: 8),
                                Text(_hasSufficientBalance ? 'Pay ₹${widget.billData['rechargeAmount']}' : 'Add Money to Wallet', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                              ],
                            ),
                          ),
                        ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text('100% Safe & Secure NETC Fastag Payment', style: TextStyle(fontSize: 12, color: Colors.grey.shade400)),
                      ],
                    ),

                    // ✅ Bharat Connect Logo (Transparent & Large)
